...
namespace LetsGoOutDemo.Functions
{
    public static class ServeStaticFiles
    {
        ...
        {
            string wwwroot = context.FunctionAppDirectory + "/wwwroot";

            string path = req.Path.Value.Replace("/api/ui/", "/");
            var contentType = FileMap.FirstOrDefault((kv => path.StartsWith(kv[0])));
            if(contentType != null)
            {
                return File.Exists(wwwroot + path) ? 
                    (IActionResult)new FileStreamResult(File.OpenRead(wwwroot + path), contentType[1]) : 
                    new NotFoundResult();
            }
